...
def training_step(images:torch.Tensor, labels, model: nn.Module, criterion:nn.Module, device: torch.device, optimizer:torch.optim)-> Tuple[float, float, float]:
...
def train(batch_size:int, epochs:int, lr:float)->None: 
    ...
    mytable = wandb.Table( 
         columns=["images", "label", "Predictions", "epoch"]
    )
    ...
    for epoch in epochs:
        ...
        with torch.no_grad():
            ...
            for idx in random_indices:
                mytable.add_data(
                    wandb.Image(images[idx].numpy().cpu()),
                    labels[idx].item(), 
